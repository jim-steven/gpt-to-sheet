...
function logToConsole(message, type = 'info') {
  const timestamp = new Date().toISOString();
  const prefix = `[${timestamp}] [CHATGPT-LOGGER] `;
...
// Add these endpoints before the catch-all route handler
// Make sure they're not inside any other route handlers

// ChatGPT Log Conversation - with multiple fallback mechanisms
app.post('/api/chatgpt/log-conversation', async (req, res) => {
  logToConsole('Received request to log conversation');
  ...
  try {
    ...
    // Validate required fields
    if (!userMessage || !assistantResponse) {
      logToConsole('Missing required fields: userMessage or assistantResponse', 'error');
      return res.status(400).json({ error: 'Missing required fields', message: 'userMessage and assistantResponse are required' });
    }
    
    logToConsole(`Using spreadsheet ID: ${spreadsheetId}, Sheet name: ${sheetName}`);
    logToConsole(`User message length: ${userMessage.length}, Assistant response length: ${assistantResponse.length}`);
    
    // Generate a unique ID for this conversation
    const conversationId = crypto.randomBytes(8).toString('hex');
    logToConsole(`Generated conversation ID: ${conversationId}`);
    
    // Store the timestamp
    const timestamp = new Date().toISOString();
    
    // ATTEMPT 1: Try using the log-data-v1 endpoint logic
    logToConsole('ATTEMPT 1: Using log-data-v1 endpoint logic');
    try {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        const token = authHeader.substring(7);
        logToConsole(`Found Bearer token: ${token.substring(0, 8)}...`);
        
        // Try to get userId from token
        try {
          userId = await getUserIdByAccessToken(token);
          logToConsole(`Retrieved user ID from token: ${userId}`);
        } catch (tokenError) {
          logToConsole(`Failed to get user ID from token: ${tokenError.message}`, 'warn');
          // If token looks like a user ID, use it directly
          if (token && token.length > 8) {
            userId = token;
            logToConsole(`Using token as user ID: ${userId}`);
          }
        }
      }
      
      // If we have a userId, try to use it
      if (userId) {
        logToConsole(`Attempting to log with user ID: ${userId}`);
        
        // Create OAuth client
        const oauth2Client = createOAuth2Client();
        
        // Try to get tokens for this user
        try {
          const tokens = await getTokensByUserId(userId);
          if (tokens && tokens.access_token) {
            logToConsole('Found tokens for user, setting credentials');
            oauth2Client.setCredentials(tokens);
            ...
            logToConsole('ATTEMPT 1: Successfully logged conversation using user tokens');
            return res.status(200).json({ 
              message: 'Successfully logged conversation',
              conversationId,
              method: 'user_tokens',
              spreadsheetId,
              sheetName
            ...
            logToConsole('No valid tokens found for user', 'warn');
            throw new Error('No valid tokens found for user');
          }
        } catch (tokenError) {
          logToConsole(`Failed to get or use tokens: ${tokenError.message}`, 'warn');
          // Continue to next attempt
        }
      } else {
        logToConsole('No user ID available, moving to next attempt', 'warn');
      }
      
      // If we get here, ATTEMPT 1 failed
      throw new Error('Failed to log using user tokens');
      
    } catch (attempt1Error) {
      logToConsole(`ATTEMPT 1 FAILED: ${attempt1Error.message}`, 'error');
      // Continue to next attempt
    }
    
    // ATTEMPT 2: Try using service account
    logToConsole('ATTEMPT 2: Using service account');
    try {
      // Check if service account key file exists
      const keyFilePath = process.env.GOOGLE_SERVICE_ACCOUNT_KEY_PATH || './service-account-key.json';
      if (!fs.existsSync(keyFilePath)) {
        logToConsole(`Service account key file not found at ${keyFilePath}`, 'warn');
      ...
      logToConsole('ATTEMPT 2: Successfully logged conversation using service account');
      return res.status(200).json({ 
        message: 'Successfully logged conversation using service account',
        conversationId,
      ...
      logToConsole(`ATTEMPT 2 FAILED: ${attempt2Error.message}`, 'error');
      // Continue to next attempt
    }
    
    // ATTEMPT 3: Try using application default credentials
    logToConsole('ATTEMPT 3: Using application default credentials');
    try {
      // Try to use application default credentials
      const auth = new google.auth.GoogleAuth({
        scopes: ['https://www.googleapis.com/auth/spreadsheets'],
      ...
      logToConsole('ATTEMPT 3: Successfully logged conversation using application default credentials');
      return res.status(200).json({ 
        message: 'Successfully logged conversation using application default credentials',
        conversationId,
      ...
      logToConsole(`ATTEMPT 3 FAILED: ${attempt3Error.message}`, 'error');
      // Continue to next attempt
    }
    
    // ATTEMPT 4: Try using a temporary token
    logToConsole('ATTEMPT 4: Using temporary token');
    try {
      // Create a temporary access token
      const tempToken = crypto.randomBytes(16).toString('hex');
      ...
      logToConsole(`Created temporary token: ${tempToken.substring(0, 8)}...`);
      
      // Create OAuth client with temporary token
      const oauth2Client = createOAuth2Client();
      oauth2Client.setCredentials({
        access_token: tempToken
      });
      
      // Try to use the sheets API directly
      try {
        const sheets = google.sheets({ version: 'v4', auth: oauth2Client });
        ...
        logToConsole('ATTEMPT 4: Successfully logged conversation using temporary token');
        return res.status(200).json({ 
          message: 'Successfully logged conversation using temporary token',
          conversationId,
        ...
        logToConsole(`Sheets API error with temp token: ${sheetsError.message}`, 'error');
        throw sheetsError;
      }
      
    } catch (attempt4Error) {
      logToConsole(`ATTEMPT 4 FAILED: ${attempt4Error.message}`, 'error');
      // Continue to next attempt
    }
    
    // ATTEMPT 5: Store in file system for later sync
    logToConsole('ATTEMPT 5: Storing in file system for later sync');
    try {
      // Create a directory for pending conversations if it doesn't exist
      const pendingDir = path.join(__dirname, 'pending_conversations');
      if (!fs.existsSync(pendingDir)) {
        fs.mkdirSync(pendingDir, { recursive: true });
        logToConsole(`Created pending conversations directory: ${pendingDir}`);
      }
      ...
      const filePath = path.join(pendingDir, `conversation_${conversationId}.json`);
      fs.writeFileSync(filePath, JSON.stringify(conversationData, null, 2));
      
      logToConsole(`ATTEMPT 5: Saved conversation to file: ${filePath}`);
      
      // Schedule a background sync attempt
      setTimeout(() => {
        logToConsole(`Attempting background sync for conversation: ${conversationId}`);
      ...
      logToConsole(`ATTEMPT 5 FAILED: ${attempt5Error.message}`, 'error');
      // Continue to next attempt
    }
    
    // ATTEMPT 6: Last resort - store in memory
    logToConsole('ATTEMPT 6: Storing in memory (last resort)');
    try {
      // Initialize in-memory storage if it doesn't exist
      if (!global.pendingConversations) global.pendingConversations = [];
      ...
      logToConsole(`ATTEMPT 6: Stored conversation in memory with ID: ${conversationId}`);
      
      // Try to log to database if available
      try {
        if (pool) {
          await pool.query(
            'INSERT INTO pending_conversations (id, spreadsheet_id, sheet_name, user_message, assistant_response, created_at) VALUES ($1, $2, $3, $4, $5, NOW())',
            [conversationId, spreadsheetId, sheetName, userMessage, assistantResponse]
          );
          logToConsole(`Also logged to database with ID: ${conversationId}`);
        }
      } catch (dbError) {
        logToConsole(`Database logging error (non-fatal): ${dbError.message}`, 'warn');
      }
      ...
      logToConsole(`ATTEMPT 6 FAILED: ${attempt6Error.message}`, 'error');
      // This is the absolute last resort - still return success
      return res.status(200).json({ 
        message: 'Conversation received, but encountered issues with storage',
        error: process.env.NODE_ENV === 'production' ? 'Internal server error' : attempt6Error.message,
        success: false,
        spreadsheetId,
        sheetName
      });
